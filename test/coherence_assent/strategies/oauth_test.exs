defmodule CoherenceAssent.OAuthTest do
  use CoherenceAssent.Test.ConnCase

  import OAuth2.TestHelpers
  alias CoherenceAssent.Strategies.OAuth
  ...
  describe "authorize_url/2" do
    test "returns url", %{conn: conn, config: config, bypass: bypass} do
      ...
      assert {:ok, %{conn: _conn, url: url}} = OAuth.authorize_url(conn, config)
    ...
    test "bubbles up network error", %{conn: conn, config: config, bypass: bypass} do
      ...
      assert {:error, %{conn: _conn, error: %OAuth2.Error{reason: :econnrefused}}} = OAuth.authorize_url(conn, config)
    end
  end
  ...
  describe "callback/2" do
    test "normalizes data", %{conn: conn, config: config, params: params, bypass: bypass} do
      ...
      expected = %{"email" => nil}
       ...
       {:ok, %{user: user}} = OAuth.callback(conn, config, params)
    ...
    test "bubbles up network error", %{conn: conn, config: config, params: params, bypass: bypass} do
      ...
      {:error, %{conn: _conn, error: %OAuth2.Response{body: %{"error" => "Unknown error"}}}} = OAuth.callback(conn, config, params)
    end
  end
end
